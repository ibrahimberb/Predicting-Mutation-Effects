...
def counts_baseline_vs_our_method(
        ...
        add_core_flag_1_case_dict: Union[dict, None]
):
    """
    ...
    ----------
        ...
        add_core_flag_1_case_dict : <None> or <dict>
            Controls whether to add `- - - TODO - - -` or to add +0 (i.e. ignoring `core_flag=1` case).
            If `None`, `core_flag=1` case adds +0.
            Otherwise `core_flag=1` case adds ELASPIC degree. TODO <<< This is not elaspic degree, this is num. of interface entries.
    ...
    if add_core_flag_1_case_dict:
        print('Adding  - - - TODO - - - when `core_flag=1`')
    ...
    for patient in tqdm(patients):
        ...
        for protein, mutations in get_patient_protein_to_mutations_dict(patient_snv_data).items():
            ...
            if core_flag == 1:
                # print(f'CORE_FLAG = {core_flag}')
                # print('+ Adding counts.. ', brca_proteins_to_elaspic_degree[protein]) # << TODO that's not degree, that is number of interface entries.
                # increase baseline counts by - - - TODO - - -
                # Increase our method counts by - - - TODO - - -
    ...
    return proteins_to_counts_baseline_dict, proteins_to_counts_our_method_dict

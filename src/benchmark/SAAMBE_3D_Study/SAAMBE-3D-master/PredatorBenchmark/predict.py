...
PREDATOR_BENCHMARK_DIR = "PredatorBenchmark"
TRAIN_DATA_PATH = op.join(PREDATOR_BENCHMARK_DIR, "train_data_with_PDB.csv")
...
def main():
    ...
    for index, row in train_data.iterrows():
        ...
        if output not in ["Nondisruptive", "Disruptive"]:
            error_string = ("\033[91m> Error: {} {} {} {} {} {}\n {} \033[00m".format(pdb_file, chain, residue_position,
                                                                                      wildtype_aa, mutant_aa,
                                                                                      model_type, output))
            print(error_string)
        ...
        if output_class_label == actual_class_label:
            print(
                "\033[92mPredicted: {} ({}) Actual: {} \033[00m".format(output, output_class_label, actual_class_label))
            counts["true_predictions"] += 1

        else:
            print(
                "\033[91mPredicted: {} ({}) Actual: {} \033[00m".format(output, output_class_label, actual_class_label))
    ...
    file_name = "SAAMBE_3D_predictions_{}.csv".format(file_date)
    print("Prediction data `{}`is exported.".format(op.join(PREDATOR_BENCHMARK_DIR, file_name)))
    prediction_data.to_csv(op.join(PREDATOR_BENCHMARK_DIR, file_name), index=False)
